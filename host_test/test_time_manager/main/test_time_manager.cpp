...
TEST_F(TimeManagerTest, TestInitAppliesTimezone)
{
    ...
    EXPECT_CALL(mock_system_time_, setenv(testing::StrEq("TZ"), testing::StrEq("<-04>4"), 1))
        .WillOnce(Return(0));
    EXPECT_CALL(mock_system_time_, tzset())
        .Times(1);
...
TEST_F(TimeManagerTest, TestStartSntpSuccess)
{
    ...
    EXPECT_CALL(mock_sntp_, init(_))
        .WillOnce(DoAll(SaveArg<0>(&captured_config), Return(ESP_OK)));

    EXPECT_CALL(mock_sntp_, start())
        .WillOnce(Return(ESP_OK));

    EXPECT_CALL(mock_sntp_, set_sync_interval(3600000))
        .Times(1);
...
TEST_F(TimeManagerTest, TestIsSynchronizedFallback)
{
    ...
    EXPECT_CALL(mock_system_time_, localtime_r(_, _))
        .WillOnce(DoAll(SetArgPointee<1>(mock_tm), Return(&mock_tm)));
...
TEST_F(TimeManagerTest, TestGetTimestamp)
{
    ...
    EXPECT_CALL(mock_system_time_, gettimeofday(_, nullptr))
        .WillOnce(DoAll(SetArgPointee<0>(tv), Return(0)));
...
TEST_F(TimeManagerTest, TestFormattedTime)
{
    ...
    EXPECT_CALL(mock_system_time_, time(_))
        .WillOnce(DoAll(SetArgPointee<0>(1700000000), Return(1700000000)));
    struct tm mock_tm = {};
    mock_tm.tm_year = 123;
...
TEST_F(TimeManagerTest, TestCreateTimePacket)
{
    ...
    EXPECT_CALL(mock_system_time_, gettimeofday(_, nullptr))
        .WillRepeatedly(DoAll(SetArgPointee<0>(tv), Return(0)));
    EXPECT_CALL(mock_system_time_, time(_))
        .WillRepeatedly(DoAll(SetArgPointee<0>(1700000000), Return(1700000000)));
    ...
    EXPECT_EQ(packet.sync_source, static_cast<uint8_t>(TimeSyncSource::SNTP));
...
TEST_F(TimeManagerTest, TestSyncFromTimePacket)
{
    ...
    EXPECT_CALL(mock_system_time_, settimeofday(testing::Pointer(testing::AllOf(
        testing::Field(&timeval::tv_sec, expected_tv.tv_sec),
        testing::Field(&timeval::tv_usec, expected_tv.tv_usec)
    )), nullptr)).WillOnce(Return(0));

    EXPECT_CALL(mock_system_time_, setenv(testing::StrEq("TZ"), testing::StrEq("UTC4"), 1))
        .WillOnce(Return(0));
